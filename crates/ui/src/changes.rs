...
impl Changes {
    ...
    fn render_file_header(
        ...
        div()
            ...
            // A deleted file no longer exists on disk, so open-in-files
            // would point at a dead path - hide the button.
            .when(file.status != FileStatus::Deleted, |el| {
                el.child(
                    div()
    ...
    pub fn render_header_controls(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let trailing: AnyElement = if scope == DiffScope::History {
            ...
            // The button stays visible but disabled on a truncated snapshot;
            // the tooltip says why instead of leaving the user guessing.
            let discard_truncated = discard.is_none()
                && self
                    .resolved(cx)
                    .is_some_and(|diff| diff.truncated && !diff.files.is_empty());
