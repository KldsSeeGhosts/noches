...
impl TerminalPanel {
    ...
    /// explicit); closing keeps every session alive (detach is not close).
    ...
    fn on_state_changed(&mut self, cx: &mut Context<Self>) {
        ...
        if switched {
            // `open` is the SHELL's per-key flag projected onto one shared
            // panel. A canvas-to-canvas key change (another project, or a
            // project-less device retarget) reaches only this observer - the
            // shell reconciles on selected-chat change alone. Sync visibility
            // with the destination key BEFORE the ensure below, or a drawn
            // open drawer would mint a hidden PTY for a canvas whose flag is
...
mod tests {
    ...
    /// Canvas-to-canvas key change: the destination's drawer flag (written to
    /// `AppState::terminal_panels` by the shell) must gate tab creation. With
    /// the flag closed, a switch while the drawer is open elsewhere must not
    /// leave `open` set - that projection is what mints a hidden PTY.
    #[gpui::test]
    fn canvas_switch_reconciles_open_from_the_destination_key(cx: &mut gpui::TestAppContext) {
        ...
        state.update(cx, |state, _| {
            ...
            // A's drawer is open, B's was never opened (default closed).
            state
                .terminal_panels
                .insert("space-canvas:@dev-a".into(), true);
        });
        let panel = cx.new(|cx| TerminalPanel::new(state.clone(), cx));
        panel.update(cx, |panel, cx| panel.set_open(true, cx));
        assert!(cx.read(|app| panel.read(app).is_open()));

        // Retarget the canvas A -> B: the panel key change must reconcile
        // against B's flag (closed) instead of minting a hidden PTY for B.
        state.update(cx, |state, cx| {
            state.selected_device = Some("dev-b".into());
            cx.notify();
        });
        assert!(
            !cx.read(|app| panel.read(app).is_open()),
            "switching to a canvas whose drawer flag is closed must close the drawer"
        );

        // B -> A restores A's open flag and its drawer.
        state.update(cx, |state, cx| {
            state.selected_device = Some("dev-a".into());
            cx.notify();
        });
