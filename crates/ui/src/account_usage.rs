...
pub struct AccountUsage {
    ...
    /// The composer's chat binding - scopes the context ring's usage frame so
    /// a split pane never borrows another's reading.
    chat_target: crate::state::ChatTarget,
...
impl AccountUsage {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        Self {
            ...
            state,
            target: None,
            chat_target: crate::state::ChatTarget::Selected,
            harness: None,
            loaded: false,
            last_forced: None,
            error: None,
    ...
    /// Whether the account ring has a reading to show (its own width hint for
    /// the composer layout, which otherwise would not reserve it).
    pub fn has_account_ring(&self, cx: &gpui::App) -> bool {
        self.fraction(cx).is_some()
    }

    /// The device this cluster loads/switches on (`None` = local). Exposed
    /// for tests that pin a pane's composer to a chat on another device.
    #[cfg(test)]
    pub(crate) fn target_device(&self) -> Option<&str> {
...
impl Render for AccountUsage {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let context =
            crate::context_usage::usage_for_target(self.state.read(cx), &self.chat_target);
