...
struct Relay {
    ...
    /// `/device/{id}/nudge` POSTs seen, and the subset answered 200.
    nudge_requests: Arc<AtomicUsize>,
    nudge_acks: Arc<AtomicUsize>,
    nudge_allowed: watch::Sender<bool>,
...
async fn relay(hold: Hold) -> Relay {
    ...
    let nudge_requests = Arc::new(AtomicUsize::new(0));
    let nudge_acks = Arc::new(AtomicUsize::new(0));
    let (nudge_allowed, nudge_gate) = watch::channel(false);
    ...
    let nudge_seen = nudge_requests.clone();
    let nudge_ok = nudge_acks.clone();
        ...
        loop {
            tokio::select! {
                accepted = listener.accept() => {
                    ...
                    let nudge_seen = nudge_seen.clone();
                    let nudge_ok = nudge_ok.clone();
                    let nudge_gate = nudge_gate.clone();
                    peers.spawn(async move {
                        ...
                        if !headers.contains("upgrade: websocket") {
                            ...
                            } else if headers.lines().next().unwrap().contains("/nudge") {
                                nudge_seen.fetch_add(1, Ordering::SeqCst);
                                if *nudge_gate.borrow() {
                                    nudge_ok.fetch_add(1, Ordering::SeqCst);
...
/// Headroom over one 100ms scheduler tick for a loaded parallel suite; a
/// poller checks each condition every 10ms, so this bound is contention slack,
/// not pacing.
const UNTIL_TIMEOUT: Duration = Duration::from_secs(30);

async fn until(mut condition: impl FnMut() -> bool) {
    tokio::time::timeout(UNTIL_TIMEOUT, async {
        while !condition() {
...
/// Assert `condition` holds for `span` without a regression: a violation
/// panics immediately, a clean span finishes without a fixed tail wait.
async fn steady_for(span: Duration, mut condition: impl FnMut() -> bool) {
    tokio::time::timeout(UNTIL_TIMEOUT.max(span * 4), async {
        let deadline = tokio::time::Instant::now() + span;
        loop {
            assert!(condition(), "steady-state condition broke");
    ...
    .await
    .expect("steady-state observation window exceeded");
}
...
async fn new_wake_during_stalled_backfill_does_not_block_other_admissions() {
    ...
    // prove the pending job outlives a scheduler tick instead of napping.
    steady_for(Duration::from_millis(250), || {
        lock(&relay.joins_by_chat).get("stalled") == Some(&1)
    })
    .await;
...
async fn active_outboxes_do_not_rotate_clients_even_at_capacity() {
    ...
    // The pending batch keeps every transport busy: joins stay flat across
    // several ticks, no rotation.
    steady_for(Duration::from_millis(700), || {
        relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP
            && handles
                .iter()
                .all(|h| h.sync_started.load(Ordering::Acquire))
    })
...
async fn spare_capacity_admits_waiters_without_rotating_aged_clients() {
    ...
    // Spare capacity admits without rotation: the residents' joins stay flat.
    steady_for(Duration::from_millis(300), || {
        handles
            .iter()
            .all(|h| lock(&relay.joins_by_chat).get(&h.chat_id) == Some(&1))
    })
    .await;
    host.shutdown_workers().await;
}
...
async fn saturated_protected_clients_do_not_load_cold_backlog() {
    ...
    // The backlog must stay durable, never opened: hold that across several
    // scheduler ticks rather than betting on one 1.3s nap.
    steady_for(Duration::from_millis(1300), || {
        host.inner.document_loads.load(Ordering::Relaxed) == loads
            && lock(&host.inner.handles).len() == ACTIVE_SYNC_CAP
    })
    .await;
...
async fn newer_view_displaces_one_older_view_without_reconnect_ping_pong() {
    ...
    // Exactly one retirement admits the focused view; hold it past a tick
    // instead of a fixed 1s sleep.
    until(|| {
        relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP + 1
            && handles
                .iter()
                .filter(|h| !h.sync_started.load(Ordering::Acquire))
                .count()
...
async fn focused_waiter_displaces_one_running_writer_without_stopping_local_writes() {
    ...
    until(|| lock(&fresh.chat2).as_ref().is_some_and(|c| c.caught_up())).await;
    // The handoff retires exactly one writer and admits the focus winner once
    // its teardown completes; assert the settled state, not a fixed delay.
    until(|| {
        relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP + 1
            && lock(&host.inner.handles)
                .values()
    ...
    .await;
    steady_for(Duration::from_millis(500), || {
        relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP + 1
    })
    .await;
    ...
    // An incidental read/watch must not reclaim the slot or invent user
    // focus; the state must hold across scheduler ticks, not one 300ms nap.
    let old_focus = h.last_focus.load(Ordering::Acquire);
    let reread = host.open(&h.chat_id).unwrap();
    let _read = reread.watch_messages();
    steady_for(Duration::from_millis(300), || {
        !h.sync_started.load(Ordering::Acquire) && h.last_focus.load(Ordering::Acquire) == old_focus
    })
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn rejected_host_nudge_retries_then_gives_up_bounded() {
    let _budget_guard = SYNC_TEST_LOCK.lock().await;
    let relay = relay(Hold::Backfill).await;
    relay.release.send_replace(true);
    let dir = tempfile::tempdir().unwrap();
    ...
    let workspace = workspace(store.clone());
    let host = host(store, &relay);
    // The workspace row names another device as host, so this engine only
    // nudges; its own scheduler never opens the chat.
    workspace
        .create_chat("remote-chat", None, Some("peer-host"), None, None)
        .unwrap();
    host.set_workspace(workspace);

    host.nudge_remote_host("remote-chat");
    // Every rejected attempt retries: the first burst is the single flight,
    // backoff adds more once the edge starts answering.
    until(|| relay.nudge_requests.load(Ordering::SeqCst) >= 1).await;
    relay.nudge_allowed.send_replace(true);
    until(|| relay.nudge_acks.load(Ordering::SeqCst) >= 1).await;
    assert!(relay.nudge_requests.load(Ordering::SeqCst) <= NUDGE_ATTEMPTS as usize);
    host.shutdown_workers().await;
}
...
async fn interactive_handoff_prefers_an_unprotected_slot_over_a_view() {
    ...
    let _view = fresh.watch_messages();
    until(|| lock(&fresh.chat2).as_ref().is_some_and(|c| c.caught_up())).await;
    // The unprotected slot yields to the focused view; the residents keep
    // their single joins (the freed slot may go to a background job, so the
    // total can grow past CAP + 1).
    for h in &handles[1..] {
        assert!(h.sync_started.load(Ordering::Acquire));
        assert_eq!(lock(&relay.joins_by_chat).get(h.chat_id()), Some(&1));
    }
...
async fn forty_eight_parent_and_subagent_writers_all_get_service_at_capacity() {
    ...
    // The cap stays bound while the overflow waits; no fixed nap stands in
    // for a settled dispatcher.
    until(|| relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP).await;
    steady_for(Duration::from_millis(300), || {
        relay.joins.load(Ordering::SeqCst) == ACTIVE_SYNC_CAP
    })
