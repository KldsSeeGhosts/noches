...
/// Device-room nudge pacing (`nudge_remote_host`): a nudge is the only cold
/// wake for a remote host, so a rejected POST retries on a jittered backoff
/// instead of trusting the command's escort to land unassisted. Bounded - the
/// hosted edge's overflow receipt (`*`) also schedules durable reconciliation,
/// so a give-up still drains on the host's next reconcile pass.
const NUDGE_ATTEMPTS: u32 = 4;
const NUDGE_BACKOFF_BASE: std::time::Duration = std::time::Duration::from_secs(1);
const NUDGE_BACKOFF_CAP: std::time::Duration = std::time::Duration::from_secs(8);
const NUDGE_GIVE_UP: std::time::Duration = std::time::Duration::from_secs(60);
        ...
        self.spawn_worker_on(&runtime, async move {
            ...
            for attempt in 1..=NUDGE_ATTEMPTS {
                ...
                let send = client
                    ...
                    .send()
                    .await;
                let retryable = match send {
                    Ok(res) if res.status().is_success() => {
                        tracing::info!(chat = %chat, device = %host_device, "host nudged");
                        return;
                    }
                    // 5xx (the queue-full 503 included) is transient; a 4xx is
                    // a durable refusal retrying cannot fix.
                    Ok(res) => {
                        let status = res.status();
                        tracing::warn!(chat = %chat, device = %host_device,
                            status = status.as_u16(), attempt, "nudge rejected");
                        status.is_server_error()
                    }
                    Err(err) => {
                        let err = describe_http_error(err);
                        tracing::warn!(chat = %chat, attempt, error = %err, "nudge failed");
                        true
                    }
                };
                if !retryable || attempt == NUDGE_ATTEMPTS {
                    break;
                }
                ...
                tokio::time::sleep(
                    (backoff + crate::workspace_host::join_retry_jitter()).min(remaining),
                )
                .await;
                backoff = (backoff * 2).min(NUDGE_BACKOFF_CAP);
            }
            // The host drains on its next reconcile regardless; this log just
            // keeps the empty-nudge-queue shape greppable.
            tracing::warn!(chat = %chat, device = %host_device,
                "nudge gave up; host drains on reconcile");
        });
    }
