...
impl AgentAccounts {
    ...
    async fn activate_claude(&self, slot: &Slot) -> Result<(), EngineError> {
        ...
        //
        // Prepare the merged config BEFORE touching credentials, so a config
        // that can't be merged leaves the live login fully intact.
        ...
        // our side must never be readable as "empty config". A failed write
        // restores the prior credentials, keeping the swap all-or-nothing.
        if let Err(error) = write_file_atomic(file, merged.to_string().as_bytes(), false) {
            if let Some(live) = live {
                let _ = self.write_claude_credentials(&live).await;
            }
            return Err(error);
        }
        Ok(())
    ...
    async fn cursor_usage(&self, slot: &Slot) -> Result<UsageSnapshot, ProbeError> {
        ...
        let http = crate::http_error::client_builder_for(&backend)
            .timeout(HTTP_TIMEOUT)
            .build()
            .unwrap_or_else(|_| self.inner.http.clone());
...
mod login_tests {
    ...
    /// An unparseable ~/.claude.json must fail the switch BEFORE the live
    /// credential blob is touched - the error reads "not switching" and the
    /// sign-in stays exactly as it was.
    #[tokio::test]
    async fn a_claude_switch_never_writes_credentials_over_a_broken_config() {
        let tmp = tempfile::tempdir().unwrap();
        let config = config(tmp.path());
        let accounts = AgentAccounts::new(config.clone());
        // The live login sits in .credentials.json; ~/.claude.json is corrupt.
        let creds_file = config.claude_config_dir.join(".credentials.json");
        write(
            &creds_file,
            r#"{"claudeAiOauth":{"accessToken":"live-token","emailAddress":"old@example.com"}}"#,
        );
        write(&config.claude_config_file, "{ not json");
        // A saved slot to switch INTO.
        let slot = Slot {
            id: slot_id_for(HarnessId::ClaudeCode, "acct-new"),
            harness: HarnessId::ClaudeCode,
            account_key: "acct-new".into(),
            profile: SlotProfile {
            ...
            credentials: serde_json::json!({ "claudeAiOauth": { "accessToken": "new-token" } }),
            claude_config: Some(serde_json::json!({
                "oauthAccount": { "accountUuid": "acct-new", "emailAddress": "new@example.com" }
            })),
            saved_at: now_ms(),
            created_at: None,
            store_key: None,
        ...
        let error = accounts
            .activate(HarnessId::ClaudeCode, &slot.id)
            .await
            .expect_err("a broken ~/.claude.json must refuse the switch");
        assert!(
            error.to_string().contains("could not be parsed"),
            "{error}"
        );
        // The credential blob is byte-for-byte what it was.
        assert_eq!(
            std::fs::read_to_string(&creds_file).unwrap(),
            r#"{"claudeAiOauth":{"accessToken":"live-token","emailAddress":"old@example.com"}}"#
        );
        // …and the corrupt config was never rewritten.
        assert_eq!(
            std::fs::read_to_string(&config.claude_config_file).unwrap(),
            "{ not json"
        );
    }
