...
#[cfg(unix)]
#[tokio::test]
async fn discard_working_tree_refuses_an_unreadable_untracked_file() {
    use std::os::unix::fs::PermissionsExt as _;

    let tmp = tempfile::tempdir().expect("tempdir");
    let repo_dir = tmp.path().join("repo");
    init_repo(&repo_dir).await;
    let locked = repo_dir.join("locked.txt");
    ...
    let error = discard_working_tree(&repos, &repo_dir, &snapshot.checksum)
        .await
        .expect_err("unverifiable content refuses the discard");
    assert!(
        error.to_string().contains("partial diff"),
        "unreadable content truncates the snapshot: {error}"
    );
    assert!(locked.exists(), "the unreadable file survives");
    assert!(
        std::fs::File::open(&locked).is_err(),
        "still chmod 000: discard never touched it"
    );
...
#[tokio::test]
async fn discard_working_tree_rejects_a_file_added_inside_an_untracked_directory() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let repo_dir = tmp.path().join("repo");
    init_repo(&repo_dir).await;
    let repos = test_repos(&tmp.path().join("data"));

    // `?? dir/` in a collapsed status listing: the confirmation snapshot must
    // still enumerate individual files so a post-confirm addition is detected.
    std::fs::create_dir(repo_dir.join("generated")).expect("untracked dir");
    std::fs::write(repo_dir.join("generated/a.txt"), "a\n").expect("untracked file");
    let snapshot = capture_diff(&repos, &repo_dir).await.expect("snapshot");
    assert!(
        snapshot
            .files
            .iter()
            .any(|file| file.path == "generated/a.txt"),
        "untracked listing must expand directories"
    );
    std::fs::write(repo_dir.join("generated/b.txt"), "b\n").expect("late file");

    let error = discard_working_tree(&repos, &repo_dir, &snapshot.checksum)
        .await
        .expect_err("a file added after confirmation is stale");
    assert!(error.to_string().contains("changed since"), "{error}");
    assert!(repo_dir.join("generated/a.txt").exists());
    assert!(repo_dir.join("generated/b.txt").exists());
}

#[tokio::test]
async fn discard_working_tree_rejects_a_binary_replaced_after_confirmation() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let repo_dir = tmp.path().join("repo");
    init_repo(&repo_dir).await;
    let repos = test_repos(&tmp.path().join("data"));

    // A binary untracked file contributes no patch text, so the checksum must
    // pin it by content: rewriting it after confirmation is a stale discard.
    std::fs::write(repo_dir.join("blob.bin"), b"\x00\x01original\x00").expect("binary file");
    let snapshot = capture_diff(&repos, &repo_dir).await.expect("snapshot");
    assert!(
        snapshot
            .files
            .iter()
            .any(|file| file.path == "blob.bin" && file.binary),
        "binary untracked file listed"
    );
    std::fs::write(repo_dir.join("blob.bin"), b"\x00\x02replacement\x00").expect("binary rewrite");

    let error = discard_working_tree(&repos, &repo_dir, &snapshot.checksum)
        .await
        .expect_err("replaced binary is stale");
    assert!(error.to_string().contains("changed since"), "{error}");
    assert_eq!(
        std::fs::read(repo_dir.join("blob.bin")).unwrap(),
        b"\x00\x02replacement\x00"
    );
}
